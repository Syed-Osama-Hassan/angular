...
export class AppComponent implements OnInit{
  ...
  public onOpenModal(employee: Employee | null, mode: string){
    ...
    if (mode === 'edit') {
      this.editEmployee = employee? employee :  this.editEmployee;
      button.setAttribute('data-target', '#updateEmployeeModal');
    }
    if (mode === 'delete') {
      this.deleteEmployee = employee? employee : this.deleteEmployee;
  ...
  public searchEmployees(key: string): void{
    ...
    for(const employee of this.employees){
      if(employee.name.toLowerCase().indexOf(key.toLowerCase()) !== -1 || employee.email.toLowerCase().indexOf(key.toLowerCase()) !== -1 || employee.jobTitle.toLowerCase().indexOf(key.toLowerCase()) !== -1){
